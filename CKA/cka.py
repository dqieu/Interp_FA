...
class CKACalculator:
    def __init__(self, model1: nn.Module, model2: nn.Module, dataloader: DataLoader,
                 ...
                 group_size: int = 512, epsilon: float = 1e-4, is_main_process: bool = True,
    ...
    def _init_values(self, all_layer_X, all_layer_Y):
        ...
        curr_hsic_matrix = torch.zeros(self.num_elements, device=self.device)
        curr_self_hsic_x = torch.zeros(1, self.num_layers_X, device=self.device)
        curr_self_hsic_y = torch.zeros(self.num_layers_Y, 1, device=self.device)
        self.hsic_matrix = AccumTensor(torch.zeros_like(curr_hsic_matrix)).to(self.device)
        self.self_hsic_x = AccumTensor(torch.zeros_like(curr_self_hsic_x)).to(self.device)
        self.self_hsic_y = AccumTensor(torch.zeros_like(curr_self_hsic_y)).to(self.device)
